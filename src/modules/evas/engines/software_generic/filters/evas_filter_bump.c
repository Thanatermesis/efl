...
/**
 * @brief Applies bump mapping to an alpha buffer using another alpha buffer as height map, outputting alpha.
 * @param cmd Filter command containing buffers and parameters.
...
/**
 * @brief Applies bump mapping to an alpha buffer using another alpha buffer as height map, outputting RGBA.
 * @param cmd Filter command containing buffers and parameters.
 * @return EINA_TRUE on success, EINA_FALSE otherwise.
 * @ingroup Evas_Filter
 */
...
/**
 * @brief Gets the appropriate CPU function for bump mapping based on output format.
 * @param cmd The filter command specifying input, output, mask, and parameters.
 * @return A function pointer to the specific bump map implementation
 *         (alpha->alpha or alpha->rgba), or NULL if parameters are invalid
 *         or the combination is unsupported.
 * @ingroup Evas_Filter
 */
...
/**
 * @brief Generates a lookup table for Phong shading based on light direction (alpha version).
 *
 * This function creates a 256x256 table where each entry represents the
 * light intensity for a given surface normal deviation relative to the
 * light source direction. The table is indexed by (dx + 127, dy + 127),
 * where dx and dy represent the components of the surface normal projected
 * onto the XY plane relative to the light direction. This version is tailored
 * for alpha-only output.
 *
 * @param phong Pointer to the 256x256 byte array to store the lookup table.
 *              Example structure: phong[y * 256 + x] = intensity.
 * @param dark The intensity value (0-255) for surfaces facing away from the light.
 * @param color The intensity value (0-255) for surfaces directly facing the light (diffuse color).
 * @param white The intensity value (0-255) for specular highlights (currently unused).
 * @param sf Specular factor (currently unused).
 * @ingroup Evas_Filter
 */
static void
 ...
 * @brief Applies bump mapping to an alpha buffer using another alpha buffer as height map, outputting alpha.
 *
 * This function simulates lighting on a surface defined by the input alpha channel (`cmd->input`),
 * using a height map provided by the mask alpha channel (`cmd->mask`). The result is
 * written to the output alpha channel (`cmd->output`). It uses a precomputed Phong
 * lookup table generated by _phong_alpha_generate() for efficiency.
 *
 * @param cmd The filter command containing input, mask, output buffers, and bump map parameters
 *            (light angles, colors, specular factor).
 * @return EINA_TRUE on success, EINA_FALSE on failure (e.g., memory allocation, invalid buffers).
 * @ingroup Evas_Filter
 */
static Eina_Bool
 ...
 * This function simulates lighting on a surface defined by the input alpha channel (`cmd->input`),
 * using a height map provided by the mask alpha channel (`cmd->mask`). The result is
 * written to the output RGBA buffer (`cmd->output`). Unlike the alpha-alpha version,
 * this calculates lighting per pixel using diffuse (N.L) and specular (N.H) components,
 * considering elevation and compensation factors.
 *
 * @param cmd The filter command containing input, mask, output buffers, and bump map parameters
 *            (light angles, colors, specular factor, elevation, compensate flag).
