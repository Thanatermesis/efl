...
/**
 * @brief Represents a single benchmark test run configuration.
 *
 * This structure holds all the necessary information for a specific
 * benchmark test, including the callback function, its name, and the
 * parameters for iterating the test.
 */
struct _Eina_Run
{
   EINA_INLIST; /**< Macro to make this struct usable with Eina_Inlist. */

   Eina_Benchmark_Specimens cb; /**< The callback function to be benchmarked. */
   const char *name; /**< The name of this specific benchmark test. */
   int start;       /**< The starting value for the benchmark loop. */
   int end;         /**< The ending value for the benchmark loop (exclusive). */
   int step;        /**< The increment step for the benchmark loop. */
};

/**
 * @brief Represents a benchmark suite.
 *
 * This structure holds the overall benchmark name, the specific run
 * identifier, a list of individual benchmark tests (Eina_Run), and
 * a list of generated data file names.
 */
struct _Eina_Benchmark
{
   const char *name; /**< The general name of the benchmark suite (e.g., "Eina_Hash"). */
   const char *run;  /**< The name for this specific execution run (e.g., "Lookup_Performance"). */

   Eina_Inlist *runs; /**< An inlist of Eina_Run structures, each representing a test case. */
   Eina_List *names;  /**< A list of Eina_Array, where each array contains the names of data files generated by a benchmark run.
                        *  Typically, this list will contain one Eina_Array after eina_benchmark_run() is called.
                        *  The Eina_Array itself contains C-strings (char *) which are the file paths.
                        *  Example:
                        *  bench->names -> Eina_List_Node -> Eina_Array -> ["bench_MyBench_MyRun.gnuplot", "bench_MyBench_MyRun.test1.data", ...] */
};

static int _eina_benchmark_log_dom = -1; /**< Log domain for the Eina benchmark module. */
