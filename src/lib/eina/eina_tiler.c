 ...
 * Used internally by the tiler for managing lists of rectangles.
 */
struct list_node
{
   struct list_node *next; /**< Pointer to the next node in the list, or NULL if it's the last node. */
};

/**
 * @internal
 * @brief Represents a singly linked list.
 * Used internally by the tiler to store and manage collections of rectangle nodes.
 */
struct list
{
   struct list_node *head; /**< Pointer to the first node in the list, or NULL if the list is empty. */
   struct list_node *tail; /**< Pointer to the last node in the list, or NULL if the list is empty. */
};

/**
 * @internal
 * @brief Represents a rectangle with its geometric properties.
 * This structure is used for internal calculations within the tiler.
 */
struct rect
{
   int right; /**< The x-coordinate of the right edge of the rectangle (left + width). */
   int bottom;
   int left;
   int top;    /**< The y-coordinate of the top edge of the rectangle. */
   int width;  /**< The width of the rectangle. */
   int height; /**< The height of the rectangle. */
   int area;   /**< The area of the rectangle (width * height). */
};

/**
 * @internal
 * @brief Represents a node in a list of rectangles.
 * It embeds a list_node for linking and a rect structure for the rectangle data.
 */
struct rect_node
{
   struct list_node _lst; /**< The list node structure, allowing this rect_node to be part of a list_t. */
   struct rect rect;      /**< The rectangle data. */
};

/**
 * @internal
 * @brief Manages the state of the rectangle splitting and merging logic.
 * This is a core component of the tiler's algorithm for optimizing update regions.
 */
typedef struct splitter
{
   Eina_Bool need_merge; /**< Flag indicating if the list of rectangles needs a merge operation. Set to EINA_TRUE when rectangles are deleted or when fuzzy splitting might have created mergeable candidates. */
   list_t rects;         /**< The list of rectangles currently managed by the splitter. These are the candidate rectangles for tiling. */
} splitter_t;

/**
 * @internal
 * @brief A memory pool for list_node_t structures.
 * This is used to optimize allocations and deallocations of list nodes,
 * reducing overhead from frequent malloc/free calls.
 */
typedef struct list_node_pool
{
   list_node_t *node; /**< Pointer to the head of a free list of list_node_t. Nodes are taken from here when needed. */
   int len;           /**< The current number of available nodes in the pool. */
   int max;           /**< The maximum number of nodes to keep in the pool. Excess nodes are freed. */
} list_node_pool_t;
...
/**
 * @internal
 * @brief Iterator structure for traversing rectangles generated by an Eina_Tiler.
 * This structure is not meant to be manipulated directly by users;
 * it is returned by eina_tiler_iterator_new() and operated on via
 * the Eina_Iterator interface.
 */
typedef struct _Eina_Iterator_Tiler
{
   Eina_Iterator iterator;     /**< The public Eina_Iterator interface. */
   const Eina_Tiler *tiler;    /**< A pointer to the tiler being iterated. */
   list_node_t *curr;          /**< The current node in the tiler's internal list of rectangles. */
   Eina_Rectangle r;           /**< The current rectangle being pointed to by the iterator, converted to Eina_Rectangle format. */
   EINA_MAGIC                  /**< Magic number for type checking. @see EINA_MAGIC_TILER_ITERATOR */
} Eina_Iterator_Tiler;

/**
 * @internal
 * @brief Main structure for the Eina_Tiler.
 * Holds the configuration and state of the tiler, including the area it covers,
 * tile sizes, and the internal splitter mechanism.
 */
struct _Eina_Tiler
{
   /**
    * @internal
    * @brief Defines the tile size used for merging heuristics.
    * Rectangles are merged if the resulting bounding box doesn't waste
    * too much space, relative to this tile size.
    */
   struct
   {
      int w, h; /**< Width and height of a conceptual tile. Used in fuzzy merging logic. */
   } tile;

   /**
    * @internal
    * @brief Caches the last added or deleted rectangle to avoid redundant processing.
    */
   struct
   {
      Eina_Rectangle add; /**< The last rectangle passed to eina_tiler_rect_add(). */
      Eina_Rectangle del; /**< The last rectangle passed to eina_tiler_rect_del(). */
   } last;
   Eina_Rectangle area;   /**< The total area covered by this tiler. Rectangles outside this area are clipped or ignored. */
   EINA_MAGIC             /**< Magic number for type checking. @see EINA_MAGIC_TILER */
   splitter_t splitter;   /**< The internal rectangle splitter and merger. */

   Eina_Bool rounding : 1; /**< If EINA_TRUE, rectangle coordinates and dimensions are shifted by 1 bit for some internal calculations. This can affect precision but might have been intended for specific rendering pipelines. Default is EINA_TRUE. */
   Eina_Bool strict : 1;   /**< If EINA_TRUE, during iterator creation, rectangles are rounded up to the tile grid defined by `tile.w` and `tile.h`. Default is EINA_FALSE. */
};
...
/**
 * @internal
 * @brief Initializes a rect_t structure.
 * Calculates and sets the right, bottom, and area fields based on the
 * provided x, y, width, and height.
 *
...
/**
 * @internal
 * @brief Retrieves a rect_node_t from the memory pool or allocates a new one.
 * This function attempts to reuse a node from `list_node_pool`. If the pool
 * is empty, it allocates a new `rect_node_t` using `malloc`.
 *
 * @return A pointer to an initialized rect_node_t, or NULL if allocation fails.
 *         The returned node's `_lst.next` is NULL.
 */
...
/**
 * @internal
 * @brief Concatenates one list of rectangles (`other`) to the end of another (`rects`).
 * The `other` list will be empty after this operation (its head and tail pointers
 * will be NULL). If `other` is already empty, the function does nothing.
 *
 * @param rects Pointer to the destination list.
 * @param other Pointer to the source list to be appended. This list will be emptied.
 */
 ...
 *
 * @param rects Pointer to the list to which the node will be appended.
 * @param node Pointer to the list_node_t (expected to be part of a rect_node_t)
 *             to append. The node's `next` pointer should be NULL or will be overwritten.
 */
...
/**
 * @internal
 * @brief Creates a new rectangle node from a rect_t structure and appends it to a list.
 * A new rect_node_t is obtained from the pool, initialized with the given rect_t data,
 * and then appended to the list.
 *
 * @param rects Pointer to the list to which the new rectangle will be appended.
 * @param r The rect_t data for the new rectangle.
 */
...
/**
 * @internal
 * @brief Creates a new rectangle node from x, y, w, h parameters and appends it to a list.
 * This is a convenience function that calls rect_init and then rect_list_append.
 *
 * @param rects Pointer to the list to which the new rectangle will be appended.
 * @param x The x-coordinate of the top-left corner.
 * @param y The y-coordinate of the top-left corner.
 ...
 *                   Set to 0 or negative if no intersection.
...
/**
 * @internal
 * @brief Splits rectangle `r` against `current` rectangle, adding non-overlapping parts of `r` to `dirty` list.
 * This function is used when subtracting `current` from `r`. The parts of `r` that
 * do not overlap with `current` are added as new rectangles to the `dirty` list.
 * `r` itself is effectively "destroyed" or consumed in this process, as its parts
 * are carved out.
 *
 * For example, if `r` is larger than `current` and `current` is inside `r`:
 *   .----------. r
 *   | .-----.  |
 *   | |current|  |
 *   | `-----'  |
 *   `----------'
 * This could result in up to 4 new rectangles added to `dirty` (top, bottom, left, right slices of `r` around `current`).
 *
 * @param dirty List to add the resulting rectangle fragments to.
 * @param current The rectangle to subtract from `r`.
 * @param r The rectangle being split. Its original geometry is used for calculations,
 *          but it's conceptually consumed.
 */
...
/**
 * @internal
 * @brief Calculates both the intersection (intra) and the bounding box (outer) of two rectangles.
 *
 * @param a The first rectangle.
 * @param b The second rectangle.
 * @param[out] intra Pointer to a rect_t to store the intersection rectangle.
 *                   Its area will be 0 if there's no intersection.
 * @param[out] outer Pointer to a rect_t to store the bounding box rectangle.
 */
...
enum
{
   ...
   SPLIT_FUZZY_ACTION_MERGE /**< Indicates that rectangles `a` and `b` should be merged. */
};

/**
 * @internal
 * @brief Splits rectangle `b` against rectangle `a` ("current" or "existing" rectangle)
 *        and adds non-overlapping parts of `b` to the `dirty` list.
 * This is part of the "fuzzy" splitting logic. Unlike `_split_strict`, this function
 * can also decide to merge `a` and `b` if they are close enough and aligned in one dimension.
 *
 * If `b` is split, the parts of `b` that do not overlap with `a` are added to `dirty`.
 * Rectangle `b` is modified in place to represent the overlapping part or a segment
 * that might be further processed or merged.
 *
 * @param dirty List to add newly created rectangle fragments to.
 * @param a The existing rectangle against which `b` is split or merged.
 * @param[in,out] b Pointer to the rectangle being processed. It may be modified.
 *                  Its non-overlapping parts with `a` are added to `dirty`.
 * @return An integer indicating the action taken:
 *         - `SPLIT_FUZZY_ACTION_NONE`: No direct split or merge action was taken on `b` relative to `a` in a way that produces new rects in `dirty` from `b`'s original geometry, though `b` might have been clipped.
 *         - `SPLIT_FUZZY_ACTION_SPLIT`: `b` was split, and parts were added to `dirty`. `b` is modified.
 *         - `SPLIT_FUZZY_ACTION_MERGE`: `a` and `b` are candidates for merging (specifically, if they align horizontally and `b` is split by `a` vertically, this suggests a horizontal merge of the remaining part of `b` with `a`).
 */
